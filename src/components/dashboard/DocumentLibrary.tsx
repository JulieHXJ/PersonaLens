...
export function DocumentLibrary({ reports, onOpenReport }: DocumentLibraryProps) {
  ...
  // Keep the permanent example pinned, then sort temporary reports by newest first.
  const sortedReports = [...reports].sort((a, b) => 
    a.isExample !== b.isExample
      ? (a.isExample ? -1 : 1)
    ...
    <div className="w-full max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">One permanent example report plus temporary reports for this session.</p>
        </div>
            ...
            <div className="p-5 flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider border ${report.isExample ? "bg-amber-500/10 text-amber-300 border-amber-500/20" : "bg-slate-700/30 text-slate-300 border-slate-600/40"}`}>
                    {report.isExample ? "Example" : "Session"}
                  </div>
                </div>
